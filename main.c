...
int main(){
	...
	if(_fd < 0) perror("[Main] shm error");
	ftruncate(_fd, sizeof(int));
	_T = (int*)mmap(NULL, sizeof(int), PROT_READ | PROT_WRITE, MAP_SHARED, _fd, 0);
	*_T = 0;
	CPU_ZERO(&_set);
	CPU_SET(0, &_set);
	if(sched_setaffinity(0, sizeof(cpu_set_t), &_set) == -1) perror("[Main] set affinity error");
	param.sched_priority = 2;
	if(sched_setscheduler(0, SCHED_FIFO, &param) == -1) perror("[Main] set schedluer error");
	if(sched_setparam(0, &param) == -1) perror("[Main] set param error");
